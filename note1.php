...
<body>
    <script>
        $(document).ready(function() {
            ...
            function handleQueueData(url, elementID, step, windowNum = null) {
                $.get(url, function(data) {
                    ...
                    if (parseInt(normalizedQueueNum, 10) === 0) {
                        console.log(Skipped audio / speech
                            for queue number $ {
                                queueNum
                            });
                        return;
            ...
            let previousCallCount = $("#callCount").text();

            setInterval(function() {
                let currentCallCount = $("#callCount").text();
                if (currentCallCount !== previousCallCount) {
                    previousCallCount = currentCallCount;

                    // Get the latest queue number text
                    const queueText = $("#hiddenQsdS2W1Prio h1").text().trim();
                    if (!queueText) return;

                    // Prepare and queue the message
                    const message = Client number,
                        $ {
                            queueText
                        }.Please proceed to step 2 window 1. $ {
                            queueText
                        }, to step 2 window 1.;
