...
class S3QsfModel extends CI_Model
{
    ...
    public function qsfS3UpdPendModP($id)
    {
        $this->db->where('id', $id);
        $this->db->where('status', 2);
        $this->db->where('step_id', 3);
        $this->db->update('tbl_transactions', array('step_id' => 4, 'status' => 0));
    }
    ...
    public function s3w1ProceedMod()
    {
        ...
        if ($query->num_rows() > 0) {
            ...
            $this->db->where('id', $row->id);
            $this->db->update('tbl_transactions', array('step_id' => 4, 'window_id' => 0,  'status' => 0));
        }
    }
