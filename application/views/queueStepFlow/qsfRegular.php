    ...
    <div class="row" style=" height:90%;">
      ...
      <div class="col-8">
        <div class="row h-100">
          <div class="col" style=" padding-right:0;">
            ...
            <div class="row m-0 h-100">
              <div class="card" style="background-color:#edf6f9; height:75vh;">
                <div class="card-body text-center overflow-auto" id="s2QueuesRegu">
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="col" style=" padding-right:0;">
            ...
            <div class="row m-0 h-100">
              <div class="card" style="background-color:#edf6f9; height:75vh;">
                <div class="card-body text-center overflow-auto" id="s2PendingRegu">
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="col-4 h-75">
        ...
        <div class="row m-0 h-100">
          <div class="h-100 cardCstmS2Regu" style="padding:0;">
            ...
            <div id="nowServ" class="card-body text-center h-75">
              <div class="col h-75 d-flex align-items-center justify-content-center" id="s2ServingRegu">
              </div>
            </div>
            <img src=".\assets\resources\batikBLUE.PNG" alt="" style="width:100%; margin:0; display:block;">
          </div>
        ...
        <div class="container h-50">
          <div class="row">
            <div class="col">
              <button class="btn btn-primary btn-lg regular3dBtn my-2 w-100" onclick="s2ReguBtn()" id="reguBtnID">
          ...
          <div class="row">
            <div class="col">
              <button class="btn btn-secondary btn-lg skip3dBtn my-2 w-100" onclick="s2SkipReguBtn()" id="skipBtnID">
...
<script>
  function loadPendingData() {
    $.ajax({
      url: '<?= base_url('s2PendingReguRou'); ?>', // Fetch data from server
      type: 'GET',
...
<script>
  ...
  function loadStepFlow() {
    ...
    $.ajax({
      url: '<?= base_url('s2ServingReguRou') ?>',
      type: 'GET',
      success: function(data) {
        $('#s2ServingRegu').html(data);
        if ($('#s2ServingRegu').html().trim() !== '') {
    ...
    $.ajax({
      url: '<?= base_url('s2QueuesReguRou') ?>', // AJAX URL
      type: 'GET', // Using GET method
      success: function(data) {
        $('#s2QueuesRegu').html(data);
      },
      error: function(xhr, status, error) {
...
<script>
  ...
  function s2ReguBtn() {
    ...
    $('#reguBtnCnfrmYes').off('click').on('click', function() {
      $.ajax({
        url: '<?= base_url('s2ReguBtnRou') ?>', // URL of the controller method
        type: 'POST',
        success: function(response) {
          var data = JSON.parse(response);
  ...
  function s2SkipReguBtn() {
    ...
    $('#skipBtnCnfrmYes').off('click').on('click', function() {
      $.ajax({
        url: '<?= base_url('s2SkipReguBtnRou') ?>', // URL of the controller method
        type: 'POST',
        success: function(response) {
          var data = JSON.parse(response);
