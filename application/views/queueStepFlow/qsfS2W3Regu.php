      ...
      <div class="col-8">
        <div class="row h-100">
          <div class="col" style=" padding-right:0;">
            ...
            <div class="row m-0 h-100">
              <div class="card" style="background-color:#edf6f9; height:75vh;">
                <div class="card-body text-center overflow-auto" id="s2w3QueRegu">
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="col" style=" padding-right:0;">
            ...
            <div class="row m-0 h-100">
              <div class="card" style="background-color:#edf6f9; height:75vh;">
                <div class="card-body text-center overflow-auto" id="s2w3PendRegu">
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="col-4 h-75">
        <div class="card" style="background: rgb(27, 75, 176, 0.9); border-bottom-right-radius: 20px; border-top-left-radius: 20px;">
          <div class="card-body text-center" style="border-radius:20px;">
            <h3 class="qsfHeaderFont">SERVING STEP 2 WINDOW 3</h3>
          </div>
        </div>
        ...
        <div class="row m-0 h-100">
          <div class="h-100 cardCstmS2Regu" style="padding:0;">
            ...
            <div id="nowServ" class="card-body text-center h-75">
              <div class="col h-75 d-flex align-items-center justify-content-center" id="s2w3ServRegu">
              </div>
            </div>
            <img src=".\assets\resources\batikBLUE.PNG" alt="" style="width:100%; margin:0; display:block;">
          </div>
        ...
        <div class="container h-50">
          <div class="row">
            <div class="col">
              <button class="btn btn-primary btn-lg regular3dBtn my-2 w-100" onclick="s2w3ReguBtn()" id="reguBtnID">
          ...
          <div class="row">
            <div class="col">
              <button class="btn btn-secondary btn-lg skip3dBtn my-2 w-100" onclick="s2w3SkipReguBtnRou()" id="skipBtnID">
...
<script>
  function loadPendingData() {
    $.ajax({
      url: '<?= base_url('s2w3PendReguRou'); ?>', // Fetch data from server
      type: 'GET',
  ...
  function bastaKuan(id) {
    ...
    $('#pendingBtnCnfrmYes').off('click').on('click', function() {
      $.ajax({
        url: '<?= base_url('s2UpdatePendingReguRou/') ?>' + id, // Pass ID in the URL
        type: 'POST',
        success: function(response) {
...
<script>
  ...
  function loadStepFlow() {
    ...
    $.ajax({
      url: '<?= base_url('s2w3ServReguRou') ?>',
      type: 'GET',
      success: function(data) {
        $('#s2w3ServRegu').html(data);
        if ($('#s2w3ServRegu').html().trim() !== '') {
    ...
    $.ajax({
      url: '<?= base_url('s2w3QueReguRou') ?>', // AJAX URL
      type: 'GET', // Using GET method
      success: function(data) {
        $('#s2w3QueRegu').html(data);
      },
      error: function(xhr, status, error) {
...
<script>
  ...
  function s2w3ReguBtn() {
    ...
    $('#reguBtnCnfrmYes').off('click').on('click', function() {
      $.ajax({
        url: '<?= base_url('s2w3ReguBtnRou') ?>', // URL of the controller method
        type: 'POST',
        success: function(response) {
          var data = JSON.parse(response);
  ...
  function s2w3SkipReguBtnRou() {
    ...
    $('#skipBtnCnfrmYes').off('click').on('click', function() {
      $.ajax({
        url: '<?= base_url('s2w3SkipReguBtnRou') ?>', // URL of the controller method
        type: 'POST',
        success: function(response) {
          var data = JSON.parse(response);
  ...
  function s2ProceedReguBtn() {
    ...
    $('#proceedBtnCnfrmYes').off('click').on('click', function() {
      $.ajax({
        url: '<?= base_url('s2ProceedReguBtnRou') ?>',
        type: 'POST',
        success: function(response) {
          var data = JSON.parse(response);
