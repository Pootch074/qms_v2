      ...
      <div class="col-8">
        <div class="row h-100">
          ...
          <div class="col-lg-4" style="padding-right:0;">
            ...
            <div class="row m-0 h-100">
              <div class="card" style="background-color:#edf6f9; height:75vh;">
                <div class="card-body text-center overflow-auto" id="s4w2autoDQRegu">
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="col-lg-4" style="padding-right:0;">
            ...
            <div class="row m-0 h-100">
              <div class="card" style="background-color:#edf6f9; height:75vh;">
                <div class="card-body text-center overflow-auto" id="s4w2autoDQPrio">
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="col-lg-4" style="padding-right:0;">
            ...
            <div class="row m-0 h-100">
              <div class="card" style="background-color:#edf6f9; height:75vh;">
                <div class="card-body text-center overflow-auto" id="s4w2autoDP">
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="col-4 h-100">
        <div class="card" style="background: rgb(27, 75, 176, 0.9); border-bottom-right-radius: 20px; border-top-left-radius: 20px;">
          <div class="card-body text-center" style="border-radius:20px;">
            <h3 class="qsfHeaderFont">SERVING STEP 4 WINDOW 2</h3>
          </div>
        </div>
        ...
        <div class="row m-0 h-75">
          <div class="h-100 cardCstmS3" style="padding:0;">
            ...
            <div id="nowServ" class="card-body text-center h-75">
              <div class="col h-75 d-flex align-items-center justify-content-center" id="s4w2Serving">
              </div>
            </div>
        ...
        <div class="container">
          <div class="row">
            <div class="col">
              <button class="btn btn-primary btn-lg regular3dBtn my-2 w-100" onclick="s4w2ReguBtn()" id="s4w2nextReguID">
                <i class="bi bi-people-fill" style="font-size: 2em;"></i>
              </button>
            </div>

            <div class="col">
              <button class="btn btn-danger btn-lg priority3dBtn my-2 w-100" onclick="s4w2PrioBtn()" id="s4w2nextPrioID">
          ...
          <div class="row">
            <div class="col">
              <button class="btn btn-secondary btn-lg skip3dBtn my-2 w-100" onclick="s4w2SkipBtn()" id="s4w2skipID">
                <i class="bi bi-skip-backward-fill" style="font-size: 2em;"></i>
              </button>
            </div>

            <div class="col">
              <button class="btn btn-secondary btn-lg call3dBtn my-2 w-100" onclick="s4w2CallBtn()" id="callBtnID">
                <i class="bi bi-volume-up-fill" style="font-size: 2em;"></i>
              </button>
            </div>

            <div class="col">
              <button class="btn btn-success btn-lg proceed3dBtn my-2 w-100" onclick="s4w2ProceedBtn()" id="s4w2proceedID">
...
<script>
  ...
  function bastaKuan(id) {
    ...
    $('#pendingBtnCnfrmYes').off('click').on('click', function() {
      $.ajax({
        url: '<?= base_url('qsfS4W2UpdPend/') ?>' + id,
        type: 'POST',
        success: function(response) {
...
<script>
  ...
  function loadStepFlow() {
    $.ajax({
      url: '<?= base_url('s4w2ServingRou') ?>',
      type: 'GET',
      success: function(data) {
        $('#s4w2Serving').html(data);
        if ($('#s4w2Serving').html().trim() !== '') {
          $('#s4w2nextReguID').prop('disabled', true);
          $('#s4w2nextPrioID').prop('disabled', true);
          $('#s4w2skipID').prop('disabled', false);
          $('#callBtnID').prop('disabled', false);
          $('#s4w2proceedID').prop('disabled', false);
        } else {
          $('#s4w2nextReguID').prop('disabled', false);
          $('#s4w2nextPrioID').prop('disabled', false);
          $('#s4w2skipID').prop('disabled', true);
          $('#callBtnID').prop('disabled', true);
          $('#s4w2proceedID').prop('disabled', true);
        }
      },
      error: function(xhr, status, error) {
...
<script>
  ...
  function s4w2CallBtn() {
    $.ajax({
      url: '<?= base_url('s4w2CallBtnRou') ?>',
      type: 'POST',
      success: function(response) {
        var data = JSON.parse(response);
      },
