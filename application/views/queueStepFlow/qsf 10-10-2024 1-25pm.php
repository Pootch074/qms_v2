    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        if ($('#autoDisplayServing').html().trim() !== '') {
          ...
          // If no content, ensure buttons are enabled
...
<!--
<script>
  // Select the DOM element with the ID 'reguBtnID' and assign it to the variable 'reguBtn'
  const reguBtn = document.querySelector("#reguBtnID");

  // Select the DOM element with the ID 'prioBtnID' and assign it to the variable 'prioBtn'
  const prioBtn = document.querySelector("#prioBtnID");

  // Select the DOM element with the ID 'skipBtnID' and assign it to the variable 'skipBtn'
  const skipBtn = document.querySelector("#skipBtnID");

  // Retrieve the 'disabled' state from localStorage.
  // Convert the retrieved value to a boolean by comparing it to the string 'true'
  let disabled = localStorage.getItem('disabled') === 'true';

  // Check if the 'disabled' flag is true
  if (disabled) {
    // If disabled, set the 'prioBtn' button to be disabled in the UI
    prioBtn.disabled = true;

    // Remove the click event listener 'reguBtnClick' from the 'reguBtn' button
    reguBtn.removeEventListener("click", reguBtnClick);
  }

  // Define the function 'reguBtnClick' to handle click events on the 'reguBtn' button
  function reguBtnClick() {
    // Log a message to the console indicating the 'reguBtnClick' function was called
    console.log("Clicked reguBtn");

    // Toggle the 'disabled' state by negating its current value
    disabled = !disabled;

    // If the new 'disabled' state is true
    if (disabled) {
      // Remove the 'reguBtnClick' event listener from the 'reguBtn' button
      reguBtn.removeEventListener("click", reguBtnClick);

      // Disable the 'prioBtn' button in the UI
      prioBtn.disabled = true;
    } else {
      // If 'disabled' is false, add the 'reguBtnClick' event listener back to the 'reguBtn' button
      reguBtn.addEventListener("click", reguBtnClick);

      // Enable the 'prioBtn' button in the UI
      prioBtn.disabled = false;
    }

    // Save the updated 'disabled' state back to localStorage as a string
    localStorage.setItem('disabled', disabled);
  }

  // Define the function 'prioBtnClick' to handle click events on the 'prioBtn' button
  function prioBtnClick() {
    // Log a message to the console indicating the 'prioBtnClick' function was called
    console.log("Clicked prioBtn");
  }

  // Define the function 'skipBtnClick' to handle click events on the 'skipBtn' button
  function skipBtnClick() {
    // Log a message to the console indicating the 'skipBtnClick' function was called
    console.log("Clicked skipBtn");

    // Optionally, add functionality here if needed when the skip button is clicked.
  }

  // Add the 'reguBtnClick' function as a click event listener to the 'reguBtn' button
  reguBtn.addEventListener("click", reguBtnClick);

  // Add the 'prioBtnClick' function as a click event listener to the 'prioBtn' button
  prioBtn.addEventListener("click", prioBtnClick);

  // Add the 'skipBtnClick' function as a click event listener to the 'skipBtn' button
  skipBtn.addEventListener("click", skipBtnClick);
</script>
            -->