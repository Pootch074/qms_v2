    ...
    <div class="row" style=" height:90%;">
      ...
      <div class="col-8">
        <div class="row h-100">
          <div class="col" style=" padding-right:0;">
            ...
            <div class="row m-0 h-100">
              <div class="card" style="background-color:#edf6f9; height:75vh;">
                <div class="card-body text-center overflow-auto" id="s2w2QueRegu">
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="col" style=" padding-right:0;">
            ...
            <div class="row m-0 h-100">
              <div class="card" style="background-color:#edf6f9; height:75vh;">
                <div class="card-body text-center overflow-auto" id="s2w2PendRegu">
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="col-4 h-75">
        ...
        <div class="row m-0 h-100">
          <div class="h-100 cardCstmS2Regu" style="padding:0;">
            ...
            <div id="nowServ" class="card-body text-center h-75">
              <div class="col h-75 d-flex align-items-center justify-content-center" id="s2w2ServRegu">
              </div>
            </div>
            <img src=".\assets\resources\batikBLUE.PNG" alt="" style="width:100%; margin:0; display:block;">
          </div>
        ...
        <div class="container h-50">
          <div class="row">
            <div class="col">
              <button class="btn btn-primary btn-lg regular3dBtn my-2 w-100" onclick="s2w2ReguBtn()" id="reguBtnID">
          ...
          <div class="row">
            <div class="col">
              <button class="btn btn-secondary btn-lg skip3dBtn my-2 w-100" onclick="s2w2SkipReguBtn()" id="skipBtnID">
                <i class="bi bi-skip-backward-fill" style="font-size: 2em;"></i>
              </button>
            </div>

            <div class="col">
              <button class="btn btn-secondary btn-lg call3dBtn my-2 w-100" onclick="s2w2CallReguBtn()" id="callBtnID">
                <i class="bi bi-volume-up-fill" style="font-size: 2em;"></i>
              </button>
            </div>

            <div class="col">
              <button class="btn btn-success btn-lg proceed3dBtn my-2 w-100" onclick="s2w2ProceedReguBtn()" id="proceedBtnID">
...
<script>
  function loadPendingData() {
    $.ajax({
      url: '<?= base_url('s2w2PendReguRou'); ?>', // Fetch data from server
      type: 'GET',
  ...
  function bastaKuan(id) {
    ...
    $('#pendingBtnCnfrmYes').off('click').on('click', function() {
      $.ajax({
        url: '<?= base_url('s2w2UpdPendReguRou/') ?>' + id, // Pass ID in the URL
        type: 'POST',
        success: function(response) {
...
<script>
  ...
  function loadStepFlow() {
    ...
    $.ajax({
      url: '<?= base_url('s2w2ServReguRou') ?>',
      type: 'GET',
      success: function(data) {
        $('#s2w2ServRegu').html(data);
        if ($('#s2w2ServRegu').html().trim() !== '') {
    ...
    $.ajax({
      url: '<?= base_url('s2w2QueReguRou') ?>', // AJAX URL
      type: 'GET', // Using GET method
      success: function(data) {
        $('#s2w2QueRegu').html(data);
      },
      error: function(xhr, status, error) {
...
<script>
  ...
  function s2w2ReguBtn() {
    ...
    $('#reguBtnCnfrmYes').off('click').on('click', function() {
      $.ajax({
        url: '<?= base_url('s2w2ReguBtnRou') ?>', // URL of the controller method
        type: 'POST',
        success: function(response) {
          var data = JSON.parse(response);
  ...
  function s2w2SkipReguBtn() {
    ...
    $('#skipBtnCnfrmYes').off('click').on('click', function() {
      $.ajax({
        url: '<?= base_url('s2w2SkipReguBtnRou') ?>', // URL of the controller method
        type: 'POST',
        success: function(response) {
          var data = JSON.parse(response);
...
<script>
  function s2w2CallReguBtn() {
    if ($('#s2w2ServRegu').html().trim() !== '' && !$('#callBtnID').prop('disabled')) {
      isCallCooldown = true;
      $('#callBtnID').prop('disabled', true);
