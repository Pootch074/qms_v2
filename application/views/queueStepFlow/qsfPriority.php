      ...
      <div class="col-8">
        <div class="row h-100">
          <div class="col" style="padding-right:0;">
            ...
            <div class="row m-0 h-100">
              <div class="card" style="background-color:#edf6f9; height:75vh;">
                <div class="card-body text-center overflow-auto" id="s2QueuesPrio">
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="col" style="padding-right:0;">
            ...
            <div class="row m-0 h-100">
              <div class="card" style="background-color:#edf6f9; height:75vh;">
                <div class="card-body text-center overflow-auto" id="s2PendingPrio">
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="col-4 h-100">
        ...
        <div class="row m-0 h-75">
          <div class="h-100 cardCstmS2Prio" style="padding:0;">
            ...
            <div id="nowServ" class="card-body text-center h-75">
              <div class="col h-75 d-flex align-items-center justify-content-center" id="s2ServingPrio">
              </div>
            </div>
            <img src=".\assets\resources\batik.PNG" alt="" style="width:100%; margin:0; display:block;">
          </div>
        ...
        <div class="container">
          <div class="row">
            ...
            <div class="col">
              <button class="btn btn-danger btn-lg priority3dBtn my-2 w-100" onclick="s2PrioBtn()" id="prioBtnID">
          ...
          <div class="row">
            <div class="col">
              <button class="btn btn-secondary btn-lg skip3dBtn my-2 w-100" onclick="s2SkipPrioBtn()" id="skipBtnID">
            ...
            <div class="col">
              <button class="btn btn-success btn-lg proceed3dBtn my-2 w-100" onclick="s2ProceedPrioBtn()" id="proceedBtnID">
...
<script>
  function loadPendingData() {
    $.ajax({
      url: '<?= base_url('s2PendingPrioRou'); ?>', // Fetch data from server
      type: 'GET',
  ...
  function bastaKuan(id) {
    ...
    $('#pendingBtnCnfrmYes').off('click').on('click', function() {
      $.ajax({
        url: '<?= base_url('s2UpdatePendingPrioRou/') ?>' + id, // Pass ID in the URL
        type: 'POST',
        success: function(response) {
...
<script>
  ...
  function loadStepFlow() {
    ...
    $.ajax({
      url: '<?= base_url('s2ServingPrioRou') ?>',
      type: 'GET',
      success: function(data) {
        $('#s2ServingPrio').html(data);
        if ($('#s2ServingPrio').html().trim() !== '') {
    ...
    $.ajax({
      url: '<?= base_url('s2QueuesPrioRou') ?>',
      type: 'GET',
      success: function(data) {
        $('#s2QueuesPrio').html(data);
      },
      error: function(xhr, status, error) {
        console.error("Error: " + error);
      }
...
<script>
  ...
  function s2PrioBtn() {
    ...
    $('#prioBtnCnfrmYes').off('click').on('click', function() {
      $.ajax({
        url: '<?= base_url('s2PrioBtnRou') ?>', // URL of the controller method
        type: 'POST',
        success: function(response) {
          var data = JSON.parse(response);
  ...
  function s2SkipPrioBtn() {
    ...
    $('#skipBtnCnfrmYes').off('click').on('click', function() {
      $.ajax({
        url: '<?= base_url('s2SkipPrioBtnRou') ?>', // URL of the controller method
        type: 'POST',
        success: function(response) {
          var data = JSON.parse(response);
