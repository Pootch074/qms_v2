                ...
                <?php if (isset($queStep) && !empty($queStep)): ?>
                    <?php foreach ($queStep as $row): ?>
                        <div class="cardCstm flex-grow-1 mb-1 d-flex align-items-center justify-content-between" style="background-color:;">
                            ...
                            <div class="col col-lg-8 h-100 d-flex align-items-center justify-content-center" id="qsdQueueNum" style="background-color:;">
                                <h1>
                                    <?php echo $row->queue_num; ?> - <?php echo $row->category; ?>
                                </h1>
                            </div>
                            <!--DISPLAY QUEUE NUMBER END-->
                        </div>
                    <?php endforeach; ?>
                <?php else: ?>
...
<script>
    $(document).ready(function() {
        loadStatusDisplay(); // Initial load
        setInterval(loadStatusDisplay, 5000); // Refresh every second
    });


    function loadStatusDisplay() {
        $.ajax({
            url: '<?= base_url('qsdDisplayServing') ?>',
            type: 'GET',
            success: function(data) {
                $('#qsdQueueNum').html(data);
            },
